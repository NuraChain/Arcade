...
platform", after rejecting the Blender renders as not high quality. The game scenes and icons are
already hand-built SVG (Task 6); everything else the platform draws moves to the same vector style,
generated by `tools/art/` where it is geometry and hand-authored where it is drawing:

- brand: the logo mark, favicon set, apple-touch icon, and a social share image
- boards: the Ludo plate (must keep `game/layout.ts`'s RIM / MARGIN / CELL fractions exactly, or
  every token lands off its square), the 52 card faces and the back (the sheet's grid IS the card
  number: column `card % 13`, row `card / 13`), and the card-table felt with its octagon alpha
- dice faces, achievement badges, group crests, and the empty-state illustrations
- each one previewed in the browser at the sizes it is used, both languages, before it replaces
  the old one; the 3D market's GLB kit is a live scene rather than an image and stays

#### Task 16b: the game tables, to the standard of `Hokm.jpg` and `Ludo.jpg`

The user's references for the play screen (the chrome around them is ours; the TABLES are the
point):

- **Hokm**: a walnut table with ornamented green felt, seat plates around it (avatar, name, cards
  held, tricks won), the trick in the middle under a medallion that names the round and whose turn
  it is, a Trump + Last trick panel, the reader's hand fanned along the bottom, a Sort action that
  reorders the hand client-side
- **Ludo**: a glossy board with raised pawns in ringed yards, arrows on the entry squares and stars
  on the safe ones, a panel per colour with four dots for tokens home, the die in the active
  player's panel, and one large Roll button with the die on it
- every figure on those screens has a producer (hand sizes, tricks, trump, round, turn, tokens home,
  the die); **Hint** and **music** are left out because nothing is behind them, the rule CLAUDE.md
